\usepackage{listings}
\usepackage{color}
% chngcntr is loaded in header.tex.
% fancyvrb / verbatim are loaded by ff.sty; tcolorbox by header.tex.

\lstloadlanguages{C,Java,sh,xml,HTML,bash}
...
% Source code highlighting. The project is C; the per-environment
% wrappers below override `language` for C++/JSON/HTML/sh blocks.
\lstset{
    language=C,
    inputencoding=utf8,
    aboveskip=2mm,
...
{
    \rm
    \hfill\lstset{language=json,#1}\minipage{0.8\linewidth}
}
{
    \endminipage\hfill\null
}
